...
#### 5.2，Spring IoC容器的lazy-init属性实现预实例化： 
...
##### 5.2.2,finishBeanFactoryInitialization处理预实例化Bean： 
...
##### 5.2.3,DefaultListableBeanFactory对配置lazy-init属性单态Bean的预实例化： 
...
##### 5.3.2,AbstractBeanFactory的getBean方法调用FactoryBean： 
...
##### 5.3.2,AbstractBeanFactory生产Bean实例对象： 
...
##### 5.3.3,工厂Bean的实现类getObject方法创建Bean实例对象： 
...
##### 5.4.2,AbstractAutowireCapableBeanFactory类对容器生成的Bean添加后置处理器： 
...
##### 5.4.3,initializeBean方法为容器产生的Bean实例对象添加BeanPostProcessor后置处理器：
...
##### 5.4.4,AdvisorAdapterRegistrationManager在Bean对象初始化后注册通知适配器： 
...
#### 5.5,Spring IoC容器autowiring实现原理： 
...
##### 5.5.1,AbstractAutoWireCapableBeanFactory对Bean实例进行属性依赖注入： 
...
##### 5.5.2,Spring IoC容器根据Bean名称或者类型进行autowiring自动依赖注入： 
...
##### 5.5.3,DefaultSingletonBeanRegistry的registerDependentBean方法对属性注入： 
